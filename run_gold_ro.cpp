...
int main()
{
	...
	int count = 0;
	char filepath[256];
	sprintf(filepath, "normal_ro/run_gold.txt");
...
void gold (double *x1, double *x2, double dx1, double dx2)
{
	...
	while (count < 100) {
		count += 1;
		a = __rosenbrock(*x1, *x2);
		b = __rosenbrock(x1_max, x2_max);
		f1 = __rosenbrock(*x1 - (1-tau) * dx1, *x2 - (1-tau) * dx2);
		f2 = __rosenbrock(*x1 - tau * dx1, *x2 - tau * dx2);
